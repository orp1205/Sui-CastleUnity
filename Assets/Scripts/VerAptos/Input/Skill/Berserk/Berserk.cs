...
public class Berserk : MonoBehaviour
{
    ...
    [SerializeField] private int baseHPRestoration = 5;
    [SerializeField] private float hpRestorationMultiplier = 0.5f;
    ...
    private IEnumerator PerformAttack()
    {
        ...
        heroStats.setHealthSkill(Mathf.RoundToInt(heroStats.getMaxHealth()*0.2f));
    ...
    private void DealDamageToEnemiesInCone()
    {
        ...
        foreach (var hitCollider in hitColliders)
        {
            if (IsInAttackCone(hitCollider.transform.position))
            {
                ...
                {
                    enemyHealth.takeDame(heroStats.GetAttack()*(Mathf.RoundToInt(heroStats.getMaxHealth()*0.2f)));
                    enemiesHit++;
                }
            }
        }
        RestorePlayerHP(enemiesHit);
    }

    private void RestorePlayerHP(int enemiesHit)
    {
        int skillLevel = heroStats.getLevelUpList()[2];
        baseHPRestoration = Mathf.RoundToInt(heroStats.getMaxHealth()*0.1f);
        int hpRestored = Mathf.RoundToInt(baseHPRestoration * skillLevel * hpRestorationMultiplier * enemiesHit);
        heroStats.setHealth(hpRestored);
        Debug.Log($"Restored {hpRestored} HP to player");
    }
